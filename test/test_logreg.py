...
#validate gradient calculation is correct 
...
#check if model can avoid exploding gradiennt with high learning rate
...
#check if model can avoid vanishing gradiennt with low learnning rate 
...
#check if loss values are decreasing generally over iterationns  
...
#check if the loss values in the final iterations are converging and not changing drastically 
...
#checks if weights are being changed 
...
#checks if make prediciton function is correctly returning values between 0 and 1 and at the same input shape  
...
#checks model accuracy meets 50% or greater 
